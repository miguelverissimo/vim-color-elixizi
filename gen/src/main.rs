...
impl<'a, W: io::Write> Writer<'a, W> {
    fn write_header(&mut self) -> io::Result<()> {
        write!(
            self.out,
            r#"" Elixizi, a colorscheme based on spring-night
"
" Author: Miguel Verissimo
" License: MIT
"
" PLEASE DO NOT MODIFY THIS FILE DIRECTLY!
" Generated by script vim-color-elixizi/gen/{source}
...
" highlights. This guard avoids `:hi clear` if elixizi is the first colorscheme.
...
let g:colors_name = 'elixizi'

let g:elixizi_italic_comments = get(g:, 'elixizi_italic_comments', 0)
let g:elixizi_kill_italic = get(g:, 'elixizi_kill_italic', 0)
let g:elixizi_kill_bold = get(g:, 'elixizi_kill_bold', 0)
let g:elixizi_highlight_terminal = get(g:, 'elixizi_highlight_terminal', 1)
let g:elixizi_cterm_italic = get(g:, 'elixizi_cterm_italic', 0)
...
let s:italic_attr = g:elixizi_kill_italic ? '' : g:elixizi_cterm_italic ? 'gui=italic cterm=italic' : 'gui=italic'
let s:bold_attr = g:elixizi_kill_bold ? '' : 'gui=bold cterm=bold'

if exists('g:elixizi_high_contrast')
    if type(g:elixizi_high_contrast) != type(0)
        echoerr 'g:elixizi_high_contrast was changed to number value. Please read README.md of vim-color-elixizi repository and set proper value'
        let g:elixizi_high_contrast = !s:gui_running && s:true_colors
    endif
else
    let g:elixizi_high_contrast = !s:gui_running && s:true_colors
endif
...
"#,
    ...
    fn write_contrast_color_variables(&mut self) -> io::Result<()> {
        ...
        for (name, color) in {
            ...
            if let ColorCode::Contrast(high, low) = color.gui {
                writeln!(
                    self.out,
                    "let s:{}_gui = g:elixizi_high_contrast ? '{}' : '{}'",
                    name, high, low
                )?;
            }
            if let ColorCode::Contrast(high, low) = color.cterm {
                writeln!(
                    self.out,
                    "let s:{}_cterm = g:elixizi_high_contrast ? {} : {}",
                    name, high, low
                )?;
            }
        }
    ...
    fn write_highlight(&mut self, highlight: &Highlight, indent: u32) -> io::Result<()> {
        ...
        let attr_item = match highlight.attr {
            ...
            HighlightAttr::CommentItalic => "g:elixizi_italic_comments ? s:italic_attr : ''",
    ...
    fn write_term_colors(&mut self) -> io::Result<()> {
        writeln!(self.out, "if g:elixizi_highlight_terminal")?;
    ...
    fn write_airline_theme_header(&mut self) -> io::Result<()> {
        ...
        write!(
            self.out,
            r#"" vim-airline theme for elixizi colorscheme
"
" Author: Miguel Verissimo
" License: MIT
"
...
let g:airline#themes#elixizi#palette = {{}}

let g:airline#themes#elixizi#palette.accents = {{
\   'red': ['{guifg}', '', {ctermfg}, '', ''],
\ }}
...
"#,
    ...
    fn write_airline_palette<'b>(
        ...
        if let Some(modified) = map.modified {
            ...
            writeln!(
                self.out,
                "let g:airline#themes#elixizi#palette.{}_modified = {{",
                name
            )?;
            writeln!(self.out, "\\   'airline_a': {},", label)?;
            writeln!(self.out, "\\   'airline_b': {},", info)?;
    ...
    fn write_airline_theme(&mut self) -> io::Result<()> {
        ...
        writeln!(
            self.out,
            "let g:airline#themes#elixizi#palette.insert_paste = {{"
        )?;
        let label =
        ...
        writeln!(
            self.out,
            "let g:airline#themes#elixizi#palette.inactive_modified = {{"
        )?;
...
fn elixizi_writer<'a, W: io::Write>(out: W) -> Writer<'a, W> {
    ...
    #[cfg_attr(rustfmt, rustfmt_skip)]
    {
        ...
        color_name!(bg_color,            contrast("#0e1b2c", "#142941"), normal(233));
        color_name!(comment_color,       normal("#8a9aad"),  normal(189));
        color_name!(fg_color,            normal("#faf9e8"),  normal(231));
        color_name!(strings_color,       normal("#aadc9f"),  normal(231));
        color_name!(function_name_color, normal("#ea945d"),  normal(189));
        color_name!(reserved_word_color, normal("#7bdcfd"),  normal(189));
        color_name!(operator_color,      normal("#a7d0e8"),  normal(189));
        color_name!(constants_color,     normal("#c7b0c4"),  normal(189));
        color_name!(numbers_color,       normal("#fe938c"),  normal(189));
        color_name!(class_color,         normal("#e9b44c"),  normal(189));
        color_name!(variable_color,      normal("#fffeec"),  normal(189));
        color_name!(arguments_color,     normal("#e0e4cc"),  normal(189));

        color_name!(finder_color,        normal("#ff3392"),  normal(189));
    }
    let table = table;
    ...
    let highlights = &[
        // Normal colors
        Always(fgbg!(Normal,                fg_color,            bg_color, Nothing)),
        Always(fgbg!(Cursor,                bg_color,            fg_color, Nothing)),
        Always(fgbg!(Comment,               comment_color,       -,        CommentItalic)),
        Always(fgbg!(Boolean,               numbers_color,       -,        Nothing)),
        Always(fgbg!(Character,             strings_color,       -,        Nothing)),
        Always(fgbg!(Conditional,           reserved_word_color, -,        Nothing)),
        Always(fgbg!(Constant,              constants_color,     -,        Nothing)),
        Always(fgbg!(Define,                reserved_word_color, -,        Nothing)),
        Always(fgbg!(Float,                 numbers_color,       -,        Nothing)),
        Always(fgbg!(Label,                 reserved_word_color, -,        Nothing)),
        Always(fgbg!(Function,              function_name_color, -,        Nothing)),
        Always(fgbg!(Identifier,            function_name_color, -,        Italic)),
        Always(fgbg!(Number,                numbers_color,       -,        Nothing)),
        Always(fgbg!(Operater,              operator_color,      -,        Nothing)),
        Always(fgbg!(PreProc,               reserved_word_color, -,        Nothing)),
        Always(fgbg!(Question,              operator_color,      -,        Nothing)),
        Always(fgbg!(Statement,             operator_color,      -,        Nothing)),
        Always(fgbg!(String,                strings_color,       -,        Nothing)),
        Always(fgbg!(Tag,                   function_name_color, -,        Nothing)),
        Always(fgbg!(Underlined,            reserved_word_color, -,        Underline)),
        Always(fgbg!(Keyword,               function_name_color, -,        Bold)),
        Always(fgbg!(Type,                  class_color,         -,        Nothing)),
        ...
        Always(fgbg!(htmlTagName,           function_name_color, -, Nothing)),
        Always(fgbg!(rubyFunction,          function_name_color, -, Nothing)),
        Always(fgbg!(rubyIdentifier,        function_name_color, -, Nothing)),
        Always(fgbg!(elixirAtom,            constants_color,     -, Nothing)),
        Always(fgbg!(elixirArguments,       arguments_color,     -, Nothing)),
        Always(fgbg!(elixirMapDelimiter,    operator_color,      -, Nothing)),
        Always(fgbg!(elixirStructDelimiter, operator_color,      -, Nothing)),
        Always(fgbg!(elixirId,              variable_color,      -, Nothing)),
...
fn main() -> Result<(), failure::Error> {
    ...
    let opts = {
        ...
        o.optopt(
            "d",
            "dir",
            "root directory of vim-color-elixizi repository",
            "PATH",
        );
    ...
    match matches.opt_str("d") {
        Some(dir) => {
            let path = PathBuf::from(&dir).join("colors").join("elixizi.vim");
            let out = io::BufWriter::new(File::create(path)?);
            let mut writer = elixizi_writer(out);
